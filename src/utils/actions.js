...
import {modal, team_territory_counts, user, prompt_move} from "../state/state";
...
export async function runAction(move_to){
    ...
    console.log(get(team_territory_counts)[get(user).active_team.name], curr_turn.allOrNothingEnabled);
    var aon_choice = false;
    if(get(team_territory_counts)[get(user).active_team.name] == 1 && curr_turn.allOrNothingEnabled){
        aon_choice = confirm("Press Ok to wager All or Nothing, or Cancel to submit a regular move.")
    }
    let promised = await fetch(`/auth/move?target=${terr_id}&aon=${aon_choice}&timestamp=${new Date().valueOf()}`);
    if(promised.ok){
