...
<script>
  ...
  async function doNext(season, team) {
    t_data = null;
    finished_load = false;
    return getVisited(season, team)
      .then((val) => {
        t_data = val;
        finished_load = true;
      })
  ...
  function recolorMap(data) {
    ...
    data.forEach((odd) => {
      ...
      t_tag.style.fill = `var(--${odd.owner.replace(/\W/g, "")}-primary)`;
      t_tag.info = odd;
      t_tag.info.primaryColor = t_tag.style.fill;
      t_tag.info.secondaryColor = `var(--${odd.owner.replace(
        /\W/g,
        ""
      )}-secondary)`;
    });
