<script>
  ...
  import Loader from './Loader.svelte';
    ...
    function returnTime(rollTime, time){
        ...
        timeTable.seconds = Math.floor(secDiff - 60 * 60 * timeTable.hours - 60 * timeTable.minutes);
        var col = (Math.floor(secDiff) % 2 == 0)? ":":" ";
        return `${String(timeTable.hours).padStart(2, "0")}${col}${String(timeTable.minutes).padStart(2, "0")}${col}${String(timeTable.seconds).padStart(2, "0")}`;
    }
